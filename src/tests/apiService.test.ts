...
it('throws ApiError on API error response', async () => {
    ...
    await expect(apiService.get('/test', z.array(aSchema))).rejects.toThrow(ApiError);
});
...
it('throws ZodError on schema validation fail', async () => {
    ...
    await expect(apiService.get('/test', z.array(aSchema))).rejects.toThrow(z.ZodError);
});
...
it('throws generic Error if response body is not valid JSON', async () => {
    ...
    await expect(apiService.get('/test', z.array(aSchema)))
    .rejects
    .toThrow('Error formatting response to json.');
});
...
it('throws "Unknown error during request" for non-standard error', async () => {
    ...
    await expect(apiService.get('/test', z.array(aSchema)))
        .rejects
        .toThrow("Unknown error during request.");
});
